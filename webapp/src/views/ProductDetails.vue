  ...
  <v-container>
    <v-dialog v-model="dialog" width="500">
      ...
      <v-card>
        ...
        <v-card-text> This action is dangerous and irreversible! </v-card-text>

        <v-divider></v-divider>

        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" text @click="deleteProduct"> Delete </v-btn>
...
export default {
  ...
  methods: {
    ...
    deleteProduct() {
      Products.deleteProduct(this.$route.params.id).then(() => {
    ...
    attachCategoryToProduct(category) {
      Products.attachCategory(this.$route.params.id, category).then(() => {
        this.getProduct();
    ...
    detachCategoryFromProduct(category_id) {
      Products.detachCategory(this.$route.params.id, category_id).then(() => {
        this.getProduct();
      });
    },
  },
